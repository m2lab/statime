...
type StmPtpInstance = PtpInstance<BasicFilter>;
...
impl Port {
    ...
    pub fn handle_event_receive(
        &mut self,
        instance: &mut StmPtpInstance,
        data: &[u8],
        timestamp: Time,
        net: &mut impl Mutex<T = NetworkStack>,
    ) {
        let mut running_port_state = self.state.take_running();
        let actions = running_port_state.handle_event_receive(instance, data, timestamp);
    ...
    pub fn handle_general_receive(
        ...
        let mut running_port_state = self.state.take_running();
        let actions = running_port_state.handle_general_receive(instance, data);
        self.handle_port_actions(actions, net);
        self.state.set_running(running_port_state);
    }
    ...
    pub fn handle_timer(
        ...
        let actions = match timer {
            TimerName::Announce => {
                running_port_state.handle_announce_timer(instance, &mut NoForwardedTLVs)
            }
            TimerName::Sync => running_port_state.handle_sync_timer(instance),
            TimerName::DelayRequest => running_port_state.handle_delay_request_timer(instance),
    ...
    pub fn handle_send_timestamp(
        ...
        let mut running_port_state = self.state.take_running();
        let actions = running_port_state.handle_send_timestamp(instance, context, timestamp);
        self.handle_port_actions(actions, net);
        self.state.set_running(running_port_state);
    }
