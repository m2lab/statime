            ...
            futures::select_biased! {
                _ = announce_timer_delay => {
                    (ptp_instance, ptp_port).lock(|instance, port| {
                        port.handle_timer(instance.state(), TimerName::Announce, net)
                    });
                }
                _ = sync_timer_delay => {
                    (ptp_instance, ptp_port).lock(|instance, port| {
                        port.handle_timer(instance.state(), TimerName::Sync, net)
                    });
                }
                _ = delay_request_timer_delay => {
                    (ptp_instance, ptp_port).lock(|instance, port| {
                        port.handle_timer(instance.state(), TimerName::DelayRequest, net)
                    });
                }
                _ = announce_receipt_timer_delay => {
                    (ptp_instance, ptp_port).lock(|instance, port| {
                        port.handle_timer(instance.state(), TimerName::AnnounceReceipt, net)
                    });
                }
                _ = filter_update_timer_delay => {
                    (ptp_instance, ptp_port).lock(|instance, port| {
                        port.handle_timer(instance.state(), TimerName::FilterUpdate, net)
                    });
                }
