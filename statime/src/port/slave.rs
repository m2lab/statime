...
impl<A, C: Clock, F: Filter, R: Rng> Port<Running, A, R, C, F> {
    pub(super) fn send_delay_request(
        &mut self,
        ptp_instance: impl LockablePtpInstance<Filter = F>,
    ) -> PortActionIterator {
        match self.config.delay_mechanism {
            DelayMechanism::E2E { interval } => self.send_e2e_delay_request(ptp_instance, interval),
            DelayMechanism::P2P { interval } => self.send_p2p_delay_request(ptp_instance, interval),
        }
    }
...
mod tests {
    ...
    use crate::{
        ...
        port::{
            state::SlaveState,
            tests::{setup_test_instance, setup_test_port_custom_filter},
            Measurement,
        },
    };
    ...
    fn test_sync_with_delay() {
        ...
        let mut action = port.send_delay_request(&mut instance);
    ...
    fn test_peer_delay_1step() {
        ...
        let mut actions = port.send_delay_request(&mut instance);
        ...
        let mut actions = port.handle_send_timestamp(&mut instance, context, Time::from_micros(50));
