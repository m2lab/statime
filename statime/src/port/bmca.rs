...
mod tests {
    ...
    use crate::{
        ...
        port::tests::{setup_test_instance, setup_test_port},
        time::Time,
    };
    ...
    fn test_announce_receive() {
        ...
        let mut actions = port.handle_general_receive(&mut instance, packet);
    ...
    fn test_announce_receive_via_event() {
        ...
        let mut actions = port.handle_event_receive(&mut instance, packet, Time::from_micros(1));
        let Some(PortAction::ResetAnnounceReceiptTimer { .. }) = actions.next() else {
            panic!("Unexpected action");
        };
        assert!(actions.next().is_none());
        ...
        let mut actions = port.handle_event_receive(&mut instance, packet, Time::from_micros(2));
        let Some(PortAction::ResetAnnounceReceiptTimer { .. }) = actions.next() else {
            panic!("Unexpected action");
        };
        assert!(actions.next().is_none());
        ...
        let mut actions = port.handle_event_receive(&mut instance, packet, Time::from_micros(3));
        let Some(PortAction::ResetAnnounceReceiptTimer { .. }) = actions.next() else {
            panic!("Unexpected action");
        };
        assert!(actions.next().is_none());
